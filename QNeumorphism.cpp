...
void QNeumorphism::draw(QPainter *painter)
{
    ...
    if(inset_){
        ...
        shadowPainter.setCompositionMode(QPainter::CompositionMode_SourceOver);
        offset.setX(qAbs(offset.x()));
        offset.setY(qAbs(offset.y()));
        shadowPainter.drawPixmap(px.rect().adjusted(offset.x(),offset.y(),-offset.x(),-offset.y()),px);
        shadowPainter.end();

        QImage blurred(px.size(), QImage::Format_ARGB32_Premultiplied);
